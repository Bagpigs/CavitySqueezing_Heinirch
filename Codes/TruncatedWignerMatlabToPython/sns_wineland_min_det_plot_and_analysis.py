...
squeezing_vec = np.genfromtxt("sns_squeezing_vec.csv", delimiter=",")
...
# plt.plot(delta_p/2/np.pi/10**6,power_fit_1, label='power fit')
# plt.plot(-x/2/np.pi/10**6,power_func_with_const_fit,label='power_func_with_const_fit')
# plt.plot(-x/Maximal2/np.pi/10**6,laurent_fit_2, label='laurent polynomial')



### Plot wineland min over detuning
plt.plot(delta_p/2/np.pi/10**6,squeezing_vec)
# plt.xscale('log')
# plt.yscale('log')
plt.ylabel('min$_{\\theta,t} \\, \\, \\xi^2(\\theta,t)$')
plt.xlabel('$\\delta_+/2\\pi \\,\\,$(MHz)')
plt.xlim((delta_p/2/np.pi/10**6)[0],(delta_p/2/np.pi/10**6)[-1])
plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))  # or 'scientific'
plt.tight_layout()

plt.legend()
plt.savefig('plots/sns_wineland_min_det_bla.svg')
plt.show()
