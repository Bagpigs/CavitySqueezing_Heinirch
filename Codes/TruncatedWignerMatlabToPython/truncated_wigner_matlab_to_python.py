...
def eom(t, phi):
    dphidt = np.zeros(6, dtype=np.csingle)
    dphidt[0] = - 1j * chi * (2 * np.conj(phi[0]+np.sqrt(a)*phi[5]) * phi[1] * phi[2]  + np.conj(phi[1]) * (phi[0] + np.sqrt(a)*phi[5]) * phi[1]) + phi[2] * np.conj(phi[2])* (phi[0]+np.sqrt(a)*phi[5]) +    \
                gamma * (np.conj(phi[2])* phi[2] * (phi[0] + np.sqrt(a) * phi[5])  - np.conj(phi[1]) * phi[1] *(phi[0] + np.sqrt(a) * phi[5]))  -  \
                1j * chiM * (2 * np.conj(phi[0]+ np.sqrt(a)* phi[5])* phi[4]*phi[3]+np.conj(phi[3]) * (phi[0] + np.sqrt(a)*phi[5]) * phi[3] + phi[4]* (phi[0] + np.sqrt(a)* phi[5]) * np.conj(phi[4]))    +\
